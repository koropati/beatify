...
class MusicLocalDataSourceImpl implements MusicLocalDataSource {
  ...
  Future<List<SongModel>> getLocalSongs() async {
    if (kIsWeb) {
      // Local audio query doesn't work on Web
      return [];
    }

    bool hasPermission = await _checkPermission();
    if (!hasPermission) {
      throw Exception("Permission denied to read local storage");
    }
      ...
      List<SongModel> queriedSongs = await _audioQuery.querySongs(
        ...
        ignoreCase: true,
         ...
         return songs.map((song) => SongModel(
          ...
          album: song.album,
          duration: (song.duration ?? 0) ~/ 1000, // convert ms to s
          uri: song.data, // local path
          isLocal: true,
        )).toList();
      });
      return queriedSongs;
